(function () {
    ...
    // заглушить другие наши аудио-плагины (музофонд, яндекс-радио), чтобы не перекрывались
    function killOther(p) {
      if (!p) return;
      try { if (p.close) p.close(); else if (p.stop) p.stop(); } catch (e) {}
    }
    function stopOtherAudio() {
      killOther(window.muzofond_player);
      killOther(window.yradio_player);
    }
    ...
    function component() {
      ...
      this.create = function () {
        ...
        network["native"](prox + 'https://lampaplugins.github.io/store/stations.json', this.build.bind(this), function () {
    ...
    function player() {
      ...
      function stop() {
        ...
        // не audio.src='' — пустая строка резолвится в URL страницы → error-цикл
      ...
      // публичный стоп (для взаимоисключения и старта видео) — стоп + спрятать
      this.stop = function () {
        stop();
        html.toggleClass('hide', true);
      };
      this.play = function (data) {
        stopOtherAudio(); // не перекрываемся с музофондом и яндекс-радио
        stop();
    ...
    function startPlugin() {
      window.radiovera = true;
      Lampa.Component.add('radiovera', component);
      ...
      // при старте видео в Lampa — глушим радио (конфликт аудио)
      try {
        Lampa.Player.listener.follow('start', function () {
          if (window.radio_player && window.radio_player.stop) window.radio_player.stop();
        });
      } catch (e) {}
